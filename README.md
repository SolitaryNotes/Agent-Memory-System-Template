...
初始化**分两段**，两段对上下文的要求完全不同：

| 阶段 | 做什么 | 由谁做 |
|---|---|---|
| ① **机械落地** | `init.py` 复制文件、替换能自动填的占位符、生成 `AGENTS.md` / `.gitignore` | **谁都行**——零上下文要求，知道目标路径即可 |
| ② **语义填充** | 任务路由表、关键不变量、目标与阶段、目录树、待办 | **必须由目标项目里的 Agent 做** |

`init.py` 刻意把填不了的留成显式 `[TODO: ...]`。**这些 TODO 不是缺陷，是留给那个项目自己填的**——
一个不了解该项目的 Agent 硬填，填出来的只会是编造，**而那比留空更糟**（留空至少诚实）。

> 换句话说：目标项目的记忆系统，应该由**将来会持续使用它的人**来初始化。
> 那个项目的第一次会话，正是它自己路由表最合适的作者。

`init.py` 默认**跳过已存在的文件**，只有 `--force` 才覆盖，所以它不会破坏目标项目已有的内容。
